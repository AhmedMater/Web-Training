...
export class ValidatorDirective implements OnInit, OnChanges {
  ...
  createErrorTag(error: ErrorData, currentLang: LanguageEnum) {
    let errorDivID = this.formControlName + "_ValDiv_" + error.error + "_" + currentLang;
    this.translationKey = "formControls." + this.formControlName
    ...
    this.translateService.get(this.translationKey).subscribe(res => {
      ...
      if(currentLang == LanguageEnum.AR) {
  ...
  validate() {
    if(this.form != undefined) {
      ...
      const currentLang: LanguageEnum = this.localStorageService.getCurrentLanguage();
      ...
      for (let error of this.controlErrors) {
        if (this.form.get(this.formControlName).dirty
