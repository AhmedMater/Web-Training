...
public class SecurityRep {
    ...
    @Autowired
    public SecurityRep(JdbcTemplate jdbc, DataSource dataSource){
        this.jdbc = jdbc;
    ...
    public boolean isUserHasView(int userID, List<Integer> viewIDs){
        String viewIDsStr = Integer.toString(viewIDs.get(0));
        for (int i=1; i<viewIDs.size(); i++)
            viewIDsStr += ", " + Integer.toString(viewIDs.get(i));

        String sql = "SELECT user_id FROM auth_user_view WHERE user_id = ? AND view_id IN (?)";
